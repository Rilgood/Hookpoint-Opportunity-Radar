...
import { AppError, hashSecret, nowIso, safeEqual, sha256 } from '../lib.js';
...
export function authenticate(db, req, { publicRoute = false } = {}) {
  if (publicRoute) return { tenantId: config.defaultTenantId, actor: 'public', scopes: [] };
  const provided = req.headers['x-api-key'];
  if (!config.authRequired && !provided) {
    const tenantId = req.headers['x-tenant-id'] || config.defaultTenantId;
    ...
    return { tenantId, actor: 'local-development', keyId: null, scopes: ['read','write','admin'] };
  }
  ...
  return { tenantId: record.tenant_id, actor: record.name, keyId: record.id, scopes: record.scopes.split(',').map((scope) => scope.trim()).filter(Boolean) };
}
