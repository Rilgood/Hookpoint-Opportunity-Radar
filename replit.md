...
- `pnpm run db:restore-rehearsal` — backup/restore drill for the radar store: `pg_dump --schema=radar` of `DATABASE_URL` (or `SOURCE_DATABASE_URL`), restore into a scratch database (ephemeral local Postgres by default, or `SCRATCH_DATABASE_URL`), row-count comparison, then the built API in production mode against the copy must report `/api/ready` 200 with `storage_mode: postgres` and list the restored companies. Never writes to the source. The restored copy is dropped again on exit unless `KEEP_SCRATCH=true`; `pnpm run db:restore-rehearsal:test` proves that against a throwaway database on the workspace server. Runbook, production PITR path and RPO/RTO: "Backup and recovery" in `artifacts/api-server/radar-core/docs/DEPLOYMENT_AND_SECURITY.md`.
