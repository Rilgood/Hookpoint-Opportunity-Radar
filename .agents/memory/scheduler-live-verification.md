...
**How to apply:**
- `/api/v1/*` on the host needs Clerk or `X-API-Key`; for a curl smoke test insert a temporary `api_keys` row (legacy `sha256(key)` hash is accepted and upgraded) into whatever the host is actually using — when `DATABASE_URL` is set that is Postgres schema `radar` (use `psql "$DATABASE_URL"`), not the SQLite file in `artifacts/api-server/data/` — then delete it afterwards.
